  ...
  <section class="section container two-column">
    ...
    <div id="registration" class="form-panel">
      ...
      <form novalidate @submit.prevent="submitForm">
        <div class="field"><label for="name">Name</label><input id="name" v-model="form.name" type="text" autocomplete="name" :aria-invalid="attempted && !!errors.name" aria-describedby="name-error" /><p v-if="attempted && errors.name" id="name-error" class="error">{{ errors.name }}</p></div>
        <div class="field"><label for="email">Email</label><input id="email" v-model="form.email" type="email" autocomplete="email" :aria-invalid="attempted && !!errors.email" aria-describedby="email-error" /><p v-if="attempted && errors.email" id="email-error" class="error">{{ errors.email }}</p></div>
        <div class="field"><label for="riders">Number of riders</label><input id="riders" v-model.number="form.riders" type="number" min="1" max="6" step="1" :aria-invalid="attempted && !!errors.riders" aria-describedby="riders-error" /><p v-if="attempted && errors.riders" id="riders-error" class="error">{{ errors.riders }}</p></div>
